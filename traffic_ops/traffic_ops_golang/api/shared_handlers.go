...
func UpdateHandler(u Updater, db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		//create error function with ResponseWriter and Request
		handleErrs := tc.GetHandleErrorsFunc(w, r)
		//create local instance of shared typeReference pointer
		...
		decoded, errs := decodeAndValidateRequestBody(r, u)
		if len(errs) > 0 {
		...
		u = decoded.(Updater)
		log.Debugf("%++v",u)
		//now we have a validated cdn object to update
		if id != u.GetID() {
...
func CreateHandler(typeRef Inserter,db *sqlx.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		//decode the body and validate the request struct
		filled, errs := decodeAndValidateRequestBody(r, i)
		if len(errs) > 0 {
			handleErrs(http.StatusBadRequest, errs...)
			return
		}
		i = filled.(Inserter)
		log.Debugf("%++v",i)
		//now we have a validated object to insert

...
func (cdn *TOCDN) Update(db *sqlx.DB) (error, tc.ApiErrorType) {
	...
	if err != nil {
		...
		return tc.DBError,tc.SystemLevel
	}
	...
	if rowsAffected != 1 {
		if rowsAffected < 1 {
			return errors.New("no cdn found with this id"),tc.DataMissing
		} else {
			return fmt.Errorf("this update affected too many rows: %d", rowsAffected),tc.SystemLevel
		}
	}
...
func (cdn *TOCDN) Delete(db *sqlx.DB) (error, tc.ApiErrorType) {
	...
	if rowsAffected != 1 {
		if rowsAffected < 1 {
			return errors.New("no cdn with that id found"),tc.DataMissing
		} else {
			return fmt.Errorf("this create affected too many rows: %d", rowsAffected),tc.SystemLevel
		}
	}
	return nil, tc.NoError
}
